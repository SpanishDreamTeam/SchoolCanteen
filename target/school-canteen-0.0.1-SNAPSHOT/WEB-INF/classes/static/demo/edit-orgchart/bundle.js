...
/***/ function(module, exports, __webpack_require__) {
	...
	document.addEventListener('DOMContentLoaded', function () {
	  /*****************************************************************************/
  	  //此区域用于生成初始json的各种操作
	  var init_datascource = null;
	  var depth = 2;
	  if(xsd != undefined && xsd != "" && xsd != null){
		  var xotree = new XML.ObjTree();
		  var dumper = new JKL.Dumper(); 
		  var xsd_object = xotree.parseXML(xsd);
		  var pro_list = ["xs:simpleType","xs:complexType","xs:sequence","-xmlns:xs","-xmlns","-targetNamespace"];
		  deleteProperty(xsd_object,pro_list);
		  var init_datascource_str = dumper.dump(xsd_object["xs:schema"]["xs:element"][0]);
		  init_datascource_str = init_datascource_str.replace(/-name/g, "name");
		  init_datascource_str = init_datascource_str.replace(/xs:element/g, "children");
		  // 打印下初始结构
//		  console.log(init_datascource_str);
		  // 获取json深度
		  depth = getDepth(init_datascource_str);
		  // 生成json对象
		  init_datascource = eval("(" + init_datascource_str + ")");
	  }
	  if(init_datascource != null){
		  var orgchart = void 0,
	      datascource = init_datascource;
	  }else{
		  var orgchart = void 0,
		  datascource = {
		  'name': 'Ball game',
		  'children': [{ 'name': 'Football'},
					   { 'name': 'Basketball'},
					   { 'name': 'Volleyball'}]
		  };
	  }
      ...
      document.querySelector('#btn-export-hier').addEventListener('click', function () {
  	    if (!document.querySelector('pre')) {
  	      ...
  	      JSON.parse(pre.innerHTML, function(k, v) {
//  	    	  alert(k + " " + v);
  	    	  if(k == "id"){
  	    		  var node_object = $("#"+v);
  	    		  var nodes_map = {};
  	    		  var infos_map = {};
  	    		  var title 	= node_object.children("div").text();
  	    		  var basetype  = node_object.children("span").children("span.BaseType").children("select").find("option:selected").val();
  	    		  
  	    		  // 装入节点listmap
  	    		  nodes_map["-name"] = title;
  	    		  // 对不同的选择进行不同处理
	  	    	  switch(basetype){
	  	    		case "xs:string":
	  	    			var restriction = node_object.children("span").children("span.RestrictionType").children("div").children("input").val();
	  	    			infos_map["xs:restriction"] = {"-base":basetype,"xs:pattern":{"-value":restriction}};
	  	    			break;
	  	    		case "xs:integer":
	  	    			var minInclusive = node_object.children("span").children("span.RestrictionType").children("div").children("input:first-child").val();
	  	    			var maxInclusive = node_object.children("span").children("span.RestrictionType").children("div").children("input:last-child").val();
//	  	    			var restriction  = [minInclusive,maxInclusive];
	  	    			infos_map["xs:restriction"] = {"-base":basetype,"xs:minInclusive":{"-value":minInclusive},"xs:maxInclusive":{"-value":maxInclusive}};
	  	    			break;
	  	    		case "xs:boolean":
	  	    			var restriction = node_object.children("span").children("span.RestrictionType").children("div").children("select").find("option:selected").val();
	  	    			infos_map["xs:restriction"] = {"-base":basetype,"xs:choice":{"-value":restriction}};
	  	    			break;
	  	    	  }
	  	    	  // 如果未输入值，则不加simpletype
	  	    	  if(restriction == "" || minInclusive == "" || maxInclusive == ""){	 
	  	    		  nodes_map["-type"] = basetype;
	  	    	  }else{	  	    		  
	  	    		  nodes_map["xs:complexType"]  = {"xs:sequence":infos_map};
	  	    	  }
  	    		  
  	    		  nodes_arr[temp++] = nodes_map;
  	    	  }
  	    	  // 记录数字
  	    	  if(!isNaN(k) && k!=""){
  	    		  nodes_arr[temp++] = k;
  	      ...
  	      /***************************************************************************************/
  	      // 树形算法
  	      var json_object = listToJson(nodes_arr);
  	      var json_str = JSON.stringify(json_object, null, 2);
  	      // 将json串传给input
  	      var xotree = new XML.ObjTree();
  	      var dumper = new JKL.Dumper();
  	      var myjson = {"xs:schema":{"-xmlns:xs": "http://www.w3.org/2001/XMLSchema","-xmlns": "http://www.w3school.com.cn","-targetNamespace": "http://www.w3school.com.cn","xs:element":json_object["xs:element"]}};
  	      var myxsd  = formatXml(xotree.writeXML(myjson));
  	      $("#input_json").val(myxsd);
    	  console.log(myxsd);
  	    }
  	  });
	      ...
	      var n = this,
	        ...
	        if (e.children) {
	          var a = !0,
	            ...
	            p = void 0;p = f ? "addChildren" === f ? " slide-up" : "" : t >= i.depth ? " slide-up" : "", u.setAttribute("class", "node " + (e.className || "") + p), i.draggable && u.setAttribute("draggable", !0), e.parentId && u.setAttribute("data-parent", e.parentId), 
	            		u.innerHTML = '\n        <div class="title">' + e[i.nodeTitle] + '<i class="fa fa-times symbol" onclick="iLabelClick()"></i>' + "</div>\n        " + (i.nodeContent ? '<span class="BaseInfo" >\n <span class="BaseType">\n <select style="width:140px;height:25px;" onchange="addclick()">\n <option value ="xs:undefine">undefine</option>\n <option value ="xs:string">string</option>\n <option value ="xs:integer">integer</option>\n <option value="xs:boolean">boolean</option>\n </select>\n </span>\n <span class="RestrictionType" style="display:none"><div></div>\n </span>\n </span>\n' : "") + "\n      ";
	        var v = e.relationship || "";if (i.verticalDepth && t + 2 > i.verticalDepth) {
	          ...
	          if (Number(v.substr(0, 1))) {
	            ...
	            var E = document.createElement("i"),
	                ...
	                A = u.querySelector(":scope > .title");E.setAttribute("class", "edge verticalEdge bottomEdge fa"), u.appendChild(E), A.insertBefore(L, A.children[0]);
	          }
...
function addclick(){
	var BaseTypeSelect 	   = $(".BaseType").children("select");
	var RestrictionTypeDiv = $(".RestrictionType").children("div");

	//给第一个下拉框的SelectChanged时间编码 
	BaseTypeSelect.change(function (){ 
		var GetDiv = $(this).parent().parent().children("span.RestrictionType").children("div");
		//取得当前下拉框的值 
		var BaseTypeValue = $(this).val(); 
	
		if (BaseTypeValue != "") {
			GetDiv.html("");
			switch (BaseTypeValue) {
				case "xs:string":
					$('<input type="text" name="strinput" style="width:136px;height:20px;" placeholder="pattern"/>\n').appendTo(GetDiv);
					break;
				case "xs:integer":
					$('<input type="text" name="min" style="width:62px;height:20px;" placeholder="min"/>\n-<input type="text" name="max" style="width:62px;height:20px;" placeholder="max"/>\n').appendTo(GetDiv);
					break;
				case "xs:boolean":
					$('<select style="width:140px;height:25px;">\n <option value="true">True</option>\n <option value="false">False</option>\n </select>\n ').appendTo(GetDiv);
					break;
			}
			GetDiv.parent().show();
		}
...
function iLabelClick(){
	...
	iLabel.click(function(){
		...
		$("html").one("keydown",function(e){
			switch (e.which) {
				case 49:
					cur.prop("class","fa fa-asterisk symbol");
					break;
				case 50:
					cur.attr("class","fa fa-plus symbol");
					break;
				case 51:
					cur.attr("class","fa fa-minus symbol");
					break;
				case 52:
					cur.attr("class","fa fa-times symbol");
					break;
			}
		});
		switch (icon) {
			case "fa fa-times symbol":
				cur.prop("class","fa fa-asterisk symbol");
				break;
			case "fa fa-asterisk symbol":
				cur.attr("class","fa fa-plus symbol");
				break;
			case "fa fa-plus symbol":
				cur.attr("class","fa fa-minus symbol");
				break;
			case "fa fa-minus symbol":
				cur.attr("class","fa fa-times symbol");
				break;
		}
	})
...
function deleteProperty(object,pro_list){
	// 定义获取类型变量
	var gettype=Object.prototype.toString;
    for (var i in object) {
//    	console.log('node:', i);
    	// 先将单个child是map的转为数组
    	if(i === 'xs:element' && gettype.call(object[i]) != gettype.call([])){
    		object[i] = [object[i]];
    	}
    	for(var j = 0;j < pro_list.length;j++){
        	if (i === pro_list[j]) {
        		delete object[i];
        	}
        }
...
function listToJson(arr){
	...
	for(var i=0;i<arr.length;i++){
		...
		if(isNaN(arr[i])){
			...
			cur = stack.pop();
			if(tree != undefined){				
				cur["xs:element"] = tree["xs:element"];
			}
			tree = {"xs:element":[cur]};
			// 如果是叶子结点，则改为simpleType
			if(leaf){
				try {					
					tree["xs:element"] = {"-name":tree["xs:element"][0]["-name"],"xs:simpleType":tree["xs:element"][0]["xs:complexType"]["xs:sequence"]};
				} catch (e) {
					console.log("not simpleType!");
				}		
			}
			leaf = false;
		}else{
			cur = stack.pop();
			tree["xs:element"][arr[i]] = cur;
			// 如果是叶子结点，则改为simpleType
			if(leaf){
				try {					
					tree["xs:element"][arr[i]] = {"-name":tree["xs:element"][arr[i]]["-name"],"xs:simpleType":tree["xs:element"][arr[i]]["xs:complexType"]["xs:sequence"]};
				} catch (e) {
					console.log("not simpleType!");
				} 
			}
...
function setInitPro(){
	if(xsd != undefined && xsd != null && xsd != ""){
		var xotree = new XML.ObjTree();
		var dumper = new JKL.Dumper(); 
		var xsd_object = xotree.parseXML(xsd);
		var init_datascource_str = dumper.dump(xsd_object);
		// 打印下初始结构
//		console.log(init_datascource_str);
		$("div.title").each(function () {
			var cur  = $(this);
			var name = $(this).text();
			var base = cur.next().children("span.BaseType").children();
			var restriction = cur.next().children("span.RestrictionType").children();
			var key_type = false;
			var key_res  = false;
			var key_num  = false;
			var min,max;
			JSON.parse(init_datascource_str, function(k, v) {
				// 类型为number时，取出min和max
//				alert(k,v);
				if(key_num){
					var num = Number(v);
					if(!isNaN(num)){					
						max = num;
						restriction.children("input:first-child").val(min);
						restriction.children("input:last-child").val(max);
						key_num  = false;
					}
				}
				if(key_res){
					...
					if(!isNaN(num)){
						...
						// 处理布尔型的选项
						restriction.children().val(v);
						restriction.children().trigger("change");
					}else{
						// 处理字符型的选项
						restriction.children().val(v);
					}
